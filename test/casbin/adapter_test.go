...
var confpath = "/home/sin/projects/worframe/test/casbin/data/casbin_test/rbac_model.conf"
var policypath = "/home/sin/projects/worframe/test/casbin/data/casbin_test/rbac_policy.csv"
...
func initPolicy(t *testing.T, a *redisadapter.Adapter) {
	// Because the DB is empty at first,
	// so we need to load the policy from the file adapter (.CSV) first.
	e, err := casbin.NewEnforcer(confpath, policypath)
	if err != nil {
		t.Fatal(err)
	}
...
func testSaveLoad(t *testing.T, a *redisadapter.Adapter) {
	...
	e, _ := casbin.NewEnforcer(confpath, a)
	testGetPolicy(t, e, [][]string{{"alice", "data1", "read"}, {"bob", "data2", "write"}, {"data2_admin", "data2", "read"}, {"data2_admin", "data2", "write"}})
}

func testAutoSave(t *testing.T, a *redisadapter.Adapter) {
	// Initialize some policy in DB.
	initPolicy(t, a)
	// Note: you don't need to look at the above code
	// if you already have a working DB with policy inside.
...
func testFilteredPolicy(t *testing.T, a *redisadapter.Adapter) {
	// Initialize some policy in DB.
	initPolicy(t, a)
	// Note: you don't need to look at the above code
	// if you already have a working DB with policy inside.
	...
	// Create an adapter and an enforcer.
	// NewEnforcer() will load the policy automatically.
	e, _ := casbin.NewEnforcer(confpath)
	// Now set the adapter
	e.SetAdapter(a)
	...
	err = e.LoadFilteredPolicy(redisadapter.Filter{V0: []string{"alice", "bob"}})
	logErr("LoadFilteredPolicy4")
	testGetPolicy(t, e, [][]string{{"alice", "data1", "read"}, {"bob", "data2", "write"}})
}

func testRemovePolicies(t *testing.T, a *redisadapter.Adapter) {
	// Initialize some policy in DB.
	initPolicy(t, a)
	// Note: you don't need to look at the above code
	// if you already have a working DB with policy inside.
	...
	err = e.LoadFilteredPolicy(redisadapter.Filter{V0: []string{"max"}})
	logErr("LoadFilteredPolicy")
...
func testAddPolicies(t *testing.T, a *redisadapter.Adapter) {
	// Initialize some policy in DB.
	initPolicy(t, a)
	// Note: you don't need to look at the above code
	// if you already have a working DB with policy inside.
...
func testUpdatePolicies(t *testing.T, a *redisadapter.Adapter) {
	// Initialize some policy in DB.
	initPolicy(t, a)
	// Note: you don't need to look at the above code
	// if you already have a working DB with policy inside.
...
func testUpdateFilteredPolicies(t *testing.T, a *redisadapter.Adapter) {
	// Initialize some policy in DB.
	initPolicy(t, a)
	// Note: you don't need to look at the above code
	// if you already have a working DB with policy inside.
...
func TestAdapters(t *testing.T) {
	a, _ := redisadapter.NewAdapterWithPassword("tcp", "127.0.0.1:16379", "123456")
...
func TestAdapterWithOption(t *testing.T) {
	a, _ := redisadapter.NewAdapterWithOption(redisadapter.WithNetwork("tcp"), redisadapter.WithAddress("127.0.0.1:16379"), redisadapter.WithPassword("123456"))
...
func TestPoolAdapters(t *testing.T) {
	a, err := redisadapter.NewAdapterWithPool(&redis.Pool{
		Dial: func() (redis.Conn, error) {
			return redis.Dial("tcp", "127.0.0.1:16379", redis.DialPassword("123456"))
		},
	})
	if err != nil {
		t.Fatal(err)
...
func TestPoolAndOptionsAdapters(t *testing.T) {
	a, err := redisadapter.NewAdapterWithPoolAndOptions(&redis.Pool{
		Dial: func() (redis.Conn, error) {
			return redis.Dial("tcp", "127.0.0.1:16379", redis.DialPassword("123456"))
		},
	}, redisadapter.WithKey("casbin:policy:test"))
	if err != nil {
		t.Fatal(err)
	}
