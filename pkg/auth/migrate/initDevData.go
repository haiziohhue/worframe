...
func (m *DBMigrate) initDevData() error {

	err := devDeptInit(m.db)
	if err != nil {
		return err
	}

	err = devMenuInit(m.db)
	if err != nil {
		return err
	}
	err = devUserInit(m.db)
	if err != nil {
		return err
	}
	err = devRoleInit(m.db)
	if err != nil {
		return err
	}
	return nil
...
func devMenuInit(db *gorm.DB) error {
...
func devUserInit(db *gorm.DB) error {
	user := []model.SysUser{
