...
func testMenuInit(db *gorm.DB) error {
	...
	return db.Create(menuData).Error
}
func testUserInit(db *gorm.DB) error {
	user := []model.SysUser{
		{DeptId: 2, UserType: "00", UserName: "admin", NickName: "超级管理员"},
		{DeptId: 1, UserType: "00", UserName: "user_one", NickName: "普通用户"},
	}
	pwd := "123456"
